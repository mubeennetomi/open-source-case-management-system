...
export default function MonitorPage() {
  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden text-sm font-sans">
      ...
      {/* ── Nav ─────────────────────────────── */}
      <aside className={`flex flex-col bg-white border-r border-gray-100 shrink-0 transition-all duration-200 ${navCollapsed ? "w-14" : "w-44"}`}>

        {/* Logo */}
        <div className={`flex items-center border-b border-gray-100 ${navCollapsed ? "justify-center px-0 py-3.5 cursor-pointer" : "gap-2 px-4 py-3.5"}`}
          onClick={navCollapsed ? () => setNavCollapsed(false) : undefined}>
          <div className="h-7 w-7 shrink-0 rounded-md bg-black flex items-center justify-center">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M1 1L7 13L13 1" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          {!navCollapsed && <span className="font-semibold text-gray-800 text-sm tracking-tight">netomi</span>}
          {!navCollapsed && (
            <button onClick={() => setNavCollapsed(true)} className="ml-auto text-gray-300 hover:text-gray-500 transition-colors">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
          ...
          <div className="px-3 py-2.5 border-b border-gray-100">
            <div className="flex items-center justify-between gap-1 cursor-pointer">
              <div className="flex items-center gap-2 min-w-0">
                <div className="h-6 w-6 shrink-0 rounded-md bg-gray-800 flex items-center justify-center">
                  <svg width="10" height="10" viewBox="0 0 14 14" fill="none">
                    <path d="M1 1L7 13L13 1" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
                <div className="min-w-0">
                  <p className="text-xs font-medium text-gray-700 truncate leading-tight">NCW Agent</p>
                  <p className="text-[10px] text-gray-400 leading-tight">ID: 1234567890</p>
                </div>
              </div>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="text-gray-400 shrink-0">
                <path d="M3 4l3-3 3 3M3 8l3 3 3-3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
          </div>
        )}
        ...
        <nav className="flex flex-col flex-1 py-2 overflow-y-auto">
          {[
            { label: "Home", children: ["AI Agent Overview"], open: false },
            { label: "Build", open: false },
            { label: "Evaluate", open: false },
            { label: "Deploy", open: false },
            { label: "Monitor", open: true, active: true },
            { label: "Channel", open: false },
            { label: "Control", open: false },
            { label: "Settings", open: false },
          ].map(item => (
            <div key={item.label}>
              <button className={`flex items-center w-full px-3 py-1.5 text-xs transition-colors gap-1.5 ${item.active ? "text-gray-800 font-medium" : "text-gray-400 hover:text-gray-700"}`}>
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="shrink-0 text-gray-300">
                  <path d={item.open || item.active ? "M3 4.5l3 3 3-3" : "M4.5 3l3 3-3 3"} stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                {!navCollapsed && <span>{item.label}</span>}
              </button>
              {!navCollapsed && item.active && item.label === "Monitor" && (
                <div className="ml-5 my-0.5">
                  <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-amber-50 text-amber-700 text-xs font-medium">
                    <span>Agent Inbox</span>
                  </div>
                </div>
              )}
              {!navCollapsed && !item.active && item.children?.map(child => (
                <div key={child} className="ml-5 my-0.5">
                  <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-50 text-xs w-full text-left transition-colors">
                    {child}
                  </button>
                </div>
              ))}
